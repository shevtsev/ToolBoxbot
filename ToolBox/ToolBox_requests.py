...
class ToolBox(keyboards, neural_networks):
    def __init__(self):
        ...
        # Check for admin ids
        self.admin_check = lambda user_id: user_id == '206635551' or user_id == '2004851715'
...
#Private        
    ...
    def __gpt_4o(self, prompt: list[dict], message) -> tuple[int, int]:
        ...
        while True:
            ...
            if "Unexpected error during /api/chat request: Cannot connect to host www.blackbox.ai:443 ssl:True [SSLCertVerificationError: (1, '[SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate (_ssl.c:1000)')]Unexpected error during /chat/f6yhMJJ request: Cannot connect to host www.blackbox.ai:443 ssl:True [SSLCertVerificationError: (1, '[SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate (_ssl.c:1000)')]" not in response \
            and "Generated by BLACKBOX.AI, try unlimited chat https://www.blackbox.ai" not in response \
            and "Model not found or too long input. Or any other error (xD)" not in response:
                break
...
#Public
    ...
    def SomeTextsCommand(self, message, ind: int, tokens: dict[str, int]):
        ...
        for cnt, request in enumerate(requests, 1):
            if tokens["incoming_tokens"]-incoming_tokens > 0 and tokens["outgoing_tokens"]-outgoing_tokens > 0 or tokens["free_requests"]-cnt > 0 or self.admin_check(message.chat.id):
