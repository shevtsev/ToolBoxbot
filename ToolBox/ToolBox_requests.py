...
#Private        
    ...
    def __gpt_4o_mini(self, prompt: list[dict], message) -> tuple[int, int]:
        ...
        while True:
            ...
            if "Unexpected error during /api/chat request" not in response \
            and response != "Generated by BLACKBOX.AI, try unlimited chat https://www.blackbox.ai"\
            and response != "Model not found or too long input. Or any other error (xD)"\
            and "An error occurred during the request" not in response\
            and response != "Request ended with status code 404":
                break
            error_counts+=1
            if error_counts > 5:
